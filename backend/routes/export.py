...
def export_interview_kit(round_id: str, db: Session = Depends(get_db)):
    ...
    js_code = _build_docx_script(questions, jd_title, jd_company, round_title)
    ...
    filename = f"Interview_Kit_{round_title.replace(' ', '_')}.docx"
...
def _build_docx_script(questions, jd_title, jd_company, round_title):
    def esc(s):
    ...
    new Paragraph({{ spacing: {{ after: 150 }}, children: [new TextRun({{ text: "{question}", size: 22 }})] }}),
...
const doc = new Document({{
  ...
  sections: [{{
    ...
    children: [
      ...
      new Paragraph({{ spacing: {{ after: 50 }}, children: [new TextRun({{ text: "{esc(jd_title)}", size: 28, bold: true, color: "7E22CE" }})] }}),
